object Functions {
    ...
    def listToUpper(list:List[String]) = list.map(upper)

    // return the length of a string
    def length(str: String):Int = str.length

    // filter names that are less than four characters long
    def filterShortNames(names:List[String]) = names.filter((s) => s.length > 3)

    // filter names that are less than four characters long
    // and turn remaining names to uppercase
    def filterShortAndTurnToUppercase(names:List[String]) = listToUpper(filterShortNames(names))
    ...
    def toCharacterLists(list:List[String]):List[List[Char]] = list.map((x) => characters(x))

    // turn a list of strings to a list of characters
    def listOfAllCharacters(list:List[String]):List[Char] = list.flatMap((x) => characters(x))
    ...
    def longestNameInList(names:List[String]) = {
        val result = {
            if (names.isEmpty)
                ...
                names.reduce((x, xs) => if (xs.length > x.length) xs else x)
        }
        result.reverse
    }
    ...
    def joinWithSep(xs: List[String], sep: String): String = {
        xs.reduce((str, strs) => str + sep + strs)
    }
    ...
    def sumOfLists20(xs: List[List[Int]]): List[Int] = {
        ...
        def sum(list :List[Int]) = {
            list.reduce((x, xs) => x + xs)
        }
        xs.map((x) => sum(x))).filter((x) => x > 20
